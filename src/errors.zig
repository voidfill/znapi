...
const error_map = blk: {
    var arr = [_]napi_error{napi_error.generic_failure} ** std.meta.fieldNames(napi.status).len;

    for (std.meta.fieldNames(napi_error)) |name| {
        const status = @field(napi.status, name);
...
pub fn statusToError(status: napi.status) napi_error!void {
    switch (status) {
        .ok => return,
